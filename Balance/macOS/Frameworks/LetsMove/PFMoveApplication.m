...
#define kStrMoveApplicationQuestionMessage _I10NS(@"Balance must be run from the Applications folder for updates to work.\n\nI can move myself to the Applications folder if you'd like.")
...
// Helper functions
static NSString *PreferredInstallLocation(BOOL *isUserDirectory, BOOL forceRootFolder);
...
void PFMoveToApplicationsFolderIfNecessary(void) {
	...
	NSString *applicationsDirectory = PreferredInstallLocation(&installToUserApplications, YES);
...
static NSString *PreferredInstallLocation(BOOL *isUserDirectory, BOOL forceRootFolder) {
	...
	if (!forceRootFolder && [userApplicationsDirs count] > 0) {
