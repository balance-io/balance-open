...
class AccountsTabViewController: NSViewController, SectionedTableViewDelegate, SectionedTableViewDataSource, NSSearchFieldDelegate {
                ...
                async(after: institutionUpdateDelay) {
                    do {
                        try ObjC.catchException {
                            self.tableView.updateRows(oldObjects: oldData.flattened as NSArray, newObjects: self.viewModel.data.flattened as NSArray, animationOptions: [NSTableView.AnimationOptions.effectFade, NSTableView.AnimationOptions.slideDown])
                        }
                    } catch {
    ...
    @objc fileprivate func accountRemoved(_ notification: Notification) {
        ...
        if let account = notification.userInfo?[Notifications.Keys.Account] as? Account {
            ...
            do {
                try ObjC.catchException {
                    self.tableView.updateRows(oldObjects: oldData.flattened as NSArray, newObjects: viewModel.data.flattened as NSArray, animationOptions: [NSTableView.AnimationOptions.effectFade, NSTableView.AnimationOptions.slideDown])
                }
            } catch {
                self.tableView.reloadData()
            }
    ...
    func tableView(_ tableView: SectionedTableView, acceptDropFromIndex fromIndex: TableIndex, toIndex: TableIndex, dropOperation: NSTableView.DropOperation) -> Bool {
        if fromIndex.row == -1 {
            ...
            // Update the UI
            tableView.updateRows(oldObjects: oldData.flattened as NSArray, newObjects: viewModel.data.flattened as NSArray, animationOptions: NSTableView.AnimationOptions.slideDown)
