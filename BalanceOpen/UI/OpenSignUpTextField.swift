...
class OpenSignUpTextField: View, TextFieldDelegate {
    ...
    fileprivate func updatePlaceholder() {
        if let placeholderString = textField.placeholderString, placeholderString.length > 0 {
            let placeholderAttributes = [NSAttributedStringKey.foregroundColor.rawValue: placeHolderStringColor,
                                         NSAttributedStringKey.font: CurrentTheme.addAccounts.signUpFieldFont] as [AnyHashable : Any]
            textField.placeholderAttributedString = NSAttributedString(string: placeholderString, attributes: placeholderAttributes as? [NSAttributedStringKey : Any])
        }
    }
